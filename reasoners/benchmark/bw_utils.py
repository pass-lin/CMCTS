...
def load_blocksworld(
    config_file, domain_file, data_file=None, data_list=None, return_intermediate=False
):
    assert (
        data_file is not None
        and data_list is None
    ...
    for cur_instance in data_list:
        ...
        INIT, GOAL, PLAN = instance_to_text_blocksworld(
            problem, True, config_data, plan_code=gt_plan_code
        )
        cur_data["init"] = INIT
        cur_data["goal"] = GOAL
        cur_data["plan"] = PLAN
        ...
        cur_data["question"] = (
            fill_template(*instance_to_text_blocksworld(problem, False, config_data))
            + "\n"
        )
...
def text_to_plan_blocksworld(
    text, cur_instance_file, config_file, domain_pddl, plan_file, ground_flag=False
):
    ...
    for line in lines:
        ...
        readable_action = "({} {})".format(
            action, " ".join(readable_objs[: n_objs + 1])
        )
        if not ground_flag:
...
def generate_all_actions(state):
    ...
    if "hand is empty" in state:
        ...
        block_color = [
            re.search("the ([a-z]{0,10}) block is clear", b).group(1) for b in block
        ]
        for c in block_color:
            if f"the {c} block is on the table" in state:
                ...
                c_ = re.search(
                    f"the {c} block" + " is on top of the ([a-z]{0,10}) block", state
                ).group(1)
                return_list.append(
...
def extract_goals(example, return_raw=False):
    ...
    goal_statement = (
        example["question"]
        .split("[STATEMENT]")[-1]
        .split("My goal is to ")[1]
        .split("My plan is as follows")[0]
        .strip()
    )
    if return_raw:
        return goal_statement
    goals = re.findall(
        "the [a-z]{0,10} block is on top of the [a-z]{0,10} block", goal_statement
    )
    return goals
