...
def get_cot_prompt(sampled_data):
    ...
    for i, entry in enumerate(sampled_data, 1):
        formatted_examples += (
            f"Q: {entry['Facts']} {entry['claims'][0]} {entry['Query']}\n"
        )
        formatted_examples += f"A: {entry['claims'][0]} "
        for j, (claim, next_step) in enumerate(
            zip(entry["claims"][1:], entry["next_steps"][:-1]), 1
        ):
...
class ProntoQAEvaluatorFinal(Evaluator):
    def __init__(
        self,
        output_extractor=lambda x: x.terminal_state.body
        if x.terminal_state is not None
        else "",
        ...
        self.queries = [
            obj.test_example.query.split(":", 1)[1].strip() for obj in dataset_list
        ]
