...
for i, example in enumerate(
    ...
    try:
        ...
        search_algo = MCTS(
            ...
            rule_action=True,
            revise_function=world_model.revise_function,
            depth_limit=depth_limit,
    ...
    while np.any(flags):
        ...
        for i in range(len(mcts_list)):
            if flags[i]:
                ...
                if state == "Search_End":
                    path, revised_results = model_input[:]
                    for node_index, result in enumerate(revised_results):
                        node = path[node_index + 1]
                        if node.revise_flag != "initial":
                            continue
                        try:
                            if eval_acc(
                                evaluator.output_extractor(path[-1].state),
                                evaluator.output_extractor(result),
                            ):
                                node.revise_flag = "activate"
                        except:
                            pass
    ...
    for i in range(len(mcts_list)):
        ...
        try:
            ...
            for t in mcts_list[i].trace_in_each_iter:
                try:
                    output = str(evaluator.output_extractor(t[-1].state))
                    output = eval(output)
