...
from world_model import Weak12KDeepMctsWorldModel as Weak12KWorldModel
from search_config import DeepWeak12KConfig as Weak12KConfig
...
for i, example in enumerate(
    ...
    try:
        ...
        reasoner.update(question, prompt)
...
for iter in range(n_iters):
        ...
        for i, state in enumerate(states):
            if state == "step":
                ...
                for t in middle_results[i].step_outputs:
                    if len(model.tokenizer.encode(t)) < model.max_tokens * 4:
                        steps_output.append(t)
                    else:
                        if '```python' in t:
                            code = t.split("```\n")[0]+'```\n'
                            code+= "运行结果 result:%s"%t.split("result")[-1]
                            steps_output.append(code)
    ...
    def scores(
        ...
        # 更新平均分数方法的累计正确率
        acc_flag_1 = game24_utils.eval_acc(mean_reward_output, candidate)
        mean_mean_reward_correct += acc_flag_1
        # 更新总分数方法的累计正确率
        acc_flag_2 = game24_utils.eval_acc(sum_reward_output, candidate)
    ...
    for i in range(len(mcts_list)):
        ...
        try:
            ...
            if not count_flag:
                print(i,count_output, candidate,int(count_output.evalf()))
            ...
            # 更新最后一个状态最高奖励策略的正确率
            max_terminal_reward_correct += game24_utils.eval_acc(max_terminal_reward_output, candidate)
            max_terminal_confidence_correct += game24_utils.eval_acc(
                max_terminal_confidence_output, candidate
            )

            # 更新最高平均奖励路径策略的正确率
            max_reward_path_corrct += game24_utils.eval_acc(max_reward_path_output, candidate)
            max_confidence_path_corrct += game24_utils.eval_acc(max_confidence_path_output, candidate)
