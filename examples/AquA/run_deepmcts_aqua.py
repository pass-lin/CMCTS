...
reward_model_name = "/home/amax/.cache/huggingface/hub/models--Qwen--Qwen2.5-Math-PRM-7B/snapshots/0610740060112df12585d00a1c5f4624d2f59051"
...
model = DeepMCTSModel4(
    ...
    max_model_len=4096,
    temperature=1,
    gpu_memory_utilization=0.9 if native_rewards_mode else 0.6,
    ...
    for i in range(len(mcts_list)):
        try:
            ...
            for t in mcts_list[i].trace_in_each_iter:
                output = str(
                    evaluator.output_extractor(t[-1].state, examples[i]["options"])
                )
