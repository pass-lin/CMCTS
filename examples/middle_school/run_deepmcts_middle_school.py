...
model = DeepMCTSModel(
    ...
    reward_model_gpu_memory_utilization=0.9,
        ...
        for i, state in enumerate(states):
            if state == "step":
                ...
                for t in middle_results[i].step_outputs:
                    if len(model.tokenizer.encode(t)) < model.max_tokens - 10:
    ...
    for i in range(len(mcts_list)):
        try:
            ...
            if not count_flag:
                ...
                # print('-----------------------------------\n'.join(t.sub_question+ '\n'+t.sub_answer for t in paths[outputs.index(count_output)][-1].state))
                # print('*'*100)
            # 如果答案不在输出集中，记录索引
